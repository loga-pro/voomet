...
class PDFReportGenerator {
  ...
  generateDailyInventoryReport(reportData) {
    console.log('🔧 DEBUG - Daily Report Data:', JSON.stringify(reportData, null, 2));
    ...
    const { date, totalReceipts, totalDispatches, totalReturns, netChange, items } = reportData;
    ...
    if (date && date !== 'undefined' && date !== 'N/A') {
      ...
      safeDate = moment().subtract(1, 'day').format('DD/MM/YYYY');
    }
    
    const safeTotalReceipts = totalReceipts || 0;
    const safeTotalDispatches = totalDispatches || 0;
    const safeTotalReturns = totalReturns || 0;
    const safeNetChange = netChange || 0;
    const safeItems = items || [];
    
    console.log('📅 Using date:', safeDate);
    ...
    doc.setProperties({
      title: `Daily Inventory Report - ${safeDate}`,
    ...
    // Header with date
    this._addUltraPremiumHeader(doc, 'Daily Inventory Report', `Date: ${safeDate}`);
    ...
    this._addModernMetricCard(
      doc, startX + cardWidth + cardSpacing, currentY, cardWidth, cardHeight,
      'TOTAL DISPATCHES', safeTotalDispatches, 'down',
      { main: COLORS.warning, bg: COLORS.warningBg }
    );
    
    this._addModernMetricCard(
      doc, startX + (cardWidth + cardSpacing) * 2, currentY, cardWidth, cardHeight,
      'TOTAL RETURNS', safeTotalReturns, 'refresh',
      { main: COLORS.danger, bg: COLORS.dangerBg }
    );
    ...
    if (safeItems.length > 0) {
      ...
      doc.setTextColor(...COLORS.textSecondary);
      doc.setFont('helvetica', 'normal');
      const noActivityText = 'No daily activity recorded';
      const noActivityWidth = doc.getTextWidth(noActivityText);
      doc.text(noActivityText, 45 + (120 - noActivityWidth) / 2, emptyY + 20);
      
      doc.setFontSize(9);
      doc.setTextColor(...COLORS.textLight);
      const forDateText = `for date ${safeDate}`;
  ...
  /**
   * Generate Milestone Project Tasks Report
   */
  generateMilestoneReport(milestoneData) {
    console.log('🔧 DEBUG - Milestone Report Data:', JSON.stringify(milestoneData, null, 2));
    
    if (!milestoneData) {
      console.error('❌ Milestone data is undefined');
      throw new Error('Milestone data is undefined');
    }

    const { customer, projectName, startDate, endDate, emailId, tasks, flexibilityPercentage } = milestoneData;
    
    // Validate required data
    if (!customer || !projectName || !startDate || !endDate || !tasks || !Array.isArray(tasks)) {
      throw new Error('Missing required milestone data fields');
    }
    ...
    doc.setProperties({
      title: `Project Tasks Report - ${projectName}`,
      subject: 'Milestone Project Tasks and Timeline',
      author: 'Voomet Project Management System',
      creator: 'Voomet Milestone Report Generator'
    });

    // Header with project information
    this._addUltraPremiumHeader(doc, 'Project Tasks Report', `Customer: ${customer} | Project: ${projectName}`);
    ...
    this._addModernMetricCard(
      doc, startX, currentY, cardWidth, cardHeight,
      'START DATE', this._formatDate(startDate), 'chart',
      { main: COLORS.info, bg: COLORS.infoBg }
    );
    ...
    this._addModernMetricCard(
      doc, startX + cardWidth + cardSpacing, currentY, cardWidth, cardHeight,
      'END DATE', this._formatDate(endDate), 'chart',
      { main: COLORS.success, bg: COLORS.successBg }
    );
    ...
    this._addModernMetricCard(
      doc, startX + (cardWidth + cardSpacing) * 2, currentY, cardWidth, cardHeight,
      'TOTAL TASKS', tasks.length, 'up',
      { main: COLORS.primary, bg: [248, 250, 252] }
    );
    
    // Flexibility Card
    this._addModernMetricCard(
      doc, startX + (cardWidth + cardSpacing) * 3, currentY, cardWidth, cardHeight,
      'FLEXIBILITY', `${flexibilityPercentage || 0}%`, 'refresh',
      { main: COLORS.warning, bg: COLORS.warningBg }
    );
    ...
    this._addModernInfoPanel(
      doc, 15, currentY, 90, 45,
      'Project Timeline',
      `${totalDuration} days`,
      `Flexibility: ${flexibilityPercentage || 0}% buffer included`
    );
    
    // Customer Contact Info Panel
    this._addModernInfoPanel(
      doc, 110, currentY, 85, 45,
      'Customer Contact',
    ...
    if (tasks.length > 0) {
      ...
      doc.setTextColor(...COLORS.textSecondary);
      doc.setFont('helvetica', 'normal');
      const noTasksText = 'No tasks defined for this project';
      const noTasksWidth = doc.getTextWidth(noTasksText);
      doc.text(noTasksText, 45 + (120 - noTasksWidth) / 2, emptyY + 20);
      
      doc.setFontSize(9);
      doc.setTextColor(...COLORS.textLight);
      const addTasksText = 'Add tasks to create a comprehensive project timeline';
      const addTasksWidth = doc.getTextWidth(addTasksText);
      doc.text(addTasksText, 45 + (120 - addTasksWidth) / 2, emptyY + 30);
  ...
  _formatDate(dateString) {
    if (!dateString || dateString === 'undefined' || dateString === 'N/A') {
      return 'TBD';
    }
