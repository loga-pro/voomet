...
const PaymentManagement = () => {
  ...
  const exportToCSV = () => {
    const headers = [
      ...
      'Total Payments', 'Pending Amount', 'Status'
    ];
                      ...
                      currentItems.map((payment) => {
                        ...
                        return (
                          <tr key={payment._id} className="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className={`text-sm font-medium ${pendingAmount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                ₹{pendingAmount.toFixed(2)?.toLocaleString()}
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div key={payment._id} className="border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
                        ...
                        <div>
                          ...
                          <span className={`ml-1 font-medium ${pendingAmount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                            ₹{pendingAmount.toFixed(2)?.toLocaleString()}
                          </span>
                        </div>
                        <div>
                  ...
                  <div className="bg-white border rounded p-3 md:col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                      <div className="bg-blue-50 p-3 rounded border">
                        ...
                        <p className="text-lg font-bold text-blue-700">₹{viewingPayment.projectCost?.toLocaleString() || '0.00'}</p>
                      </div>
                      <div className="bg-purple-50 p-3 rounded border">
                        <span className="text-xs text-gray-500">Invoices Raised</span>
                        <p className="text-lg font-bold text-purple-700">₹{viewingPayment.totalInvoiceRaised?.toLocaleString() || '0.00'}</p>
                      </div>
                      ...
                      <div className="bg-amber-50 p-3 rounded border">
                        ...
                        <p className={`text-lg font-bold ${viewingPayment.balanceAmount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                          ₹{calculatePendingAmount(viewingPayment).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="px-3 py-2 text-left text-gray-500">Inv No.</th>
                              <th className="px-3 py-2 text-left text-gray-500">Date</th>
                              <th className="px-3 py-2 text-left text-gray-500">Value (₹)</th>
                              <th className="px-3 py-2 text-left text-gray-500">Tax (%)</th>
                          ...
                          <tbody className="divide-y divide-gray-100">
                            {viewingPayment.invoices.map((inv, idx) => (
                              <tr key={idx} className="hover:bg-gray-50">
                                ...
                                <td className="px-3 py-2 text-gray-600">{(inv.cgst + inv.sgst) || 0}%</td>
                                <td className="px-3 py-2 font-bold text-gray-900">{inv.totalWithTax?.toLocaleString() || '0.00'}</td>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="px-3 py-2 text-left text-gray-500">Reference No.</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {viewingPayment.payments.map((pmt, idx) => (
                              <tr key={idx} className="hover:bg-gray-50">
                                <td className="px-3 py-2 text-gray-600">{idx + 1}</td>
                                <td className="px-3 py-2 text-gray-600">{pmt.paymentDate ? formatDate(pmt.paymentDate) : '-'}</td>
                                <td className="px-3 py-2 font-bold text-green-700">{pmt.amount?.toLocaleString() || '0.00'}</td>
                                <td className="px-3 py-2 text-gray-600 capitalize">{pmt.paymentType?.replace('-', ' ') || 'General'}</td>
                            ...
                            <tr className="bg-green-50 border-t-2 border-green-200">
                              <td colSpan="2" className="px-3 py-3 text-sm font-bold text-gray-900">Total Payments Received</td>
