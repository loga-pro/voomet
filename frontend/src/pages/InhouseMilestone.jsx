...
const InhouseMilestone = () => {
  ...
  const fetchMilestones = async () => {
    try {
      setLoading(true);
      // Fetch both regular milestones and inhouse milestones to show all data
      const [regularResponse, inhouseResponse] = await Promise.all([
        milestonesAPI.getAll().catch(err => {
          console.error('Error fetching regular milestones:', err);
          return { data: { milestones: [] } };
        }),
        inhouseMilestonesAPI.getAll().catch(err => {
          console.error('Error fetching inhouse milestones:', err);
          return { data: { milestones: [] } };
        })
      ]);
      ...
      // Add category to each milestone for tracking
      const regularWithCategory = regularMilestones.map(m => ({ ...m, category: m.category || 'outsourced', isInhouseEdit: false }));
      const inhouseWithCategory = inhouseMilestones.map(m => ({ ...m, category: 'inhouse', isInhouseEdit: true }));

      // Combine both arrays - inhouse milestones take precedence if there's a duplicate
      const allMilestonesMap = new Map();

      // First add regular milestones
      regularWithCategory.forEach(m => {
        const key = `${m.customer}_${m.projectName}`;
        allMilestonesMap.set(key, m);
      });

      // Then add/override with inhouse milestones (these are the edited versions)
      inhouseWithCategory.forEach(m => {
        const key = `${m.customer}_${m.projectName}`;
        allMilestonesMap.set(key, m);
      });

      const allMilestones = Array.from(allMilestonesMap.values());
      setMilestones(allMilestones);

      // Extract unique values for dropdowns
      const customers = [...new Set(allMilestones.map(milestone => milestone.customer))].filter(Boolean);
      const projectNames = [...new Set(allMilestones.map(milestone => milestone.projectName))].filter(Boolean);
      const emailIds = [...new Set(allMilestones.map(milestone => milestone.emailId))].filter(Boolean);
  ...
  const exportToCSV = () => {
    try {
      const headers = [
        ...
        'Start Date', 'End Date', 'Responsible Person', 'Category'
      ];
  ...
  const confirmDelete = async () => {
    if (milestoneToDelete) {
      try {
        const milestone = milestones.find(m => m._id === milestoneToDelete);
        // Only delete from inhouse DB if it's an inhouse edit
        // Regular milestones should not be deleted from here
        if (milestone && milestone.isInhouseEdit === true) {
          await inhouseMilestonesAPI.delete(milestoneToDelete);
          showSuccess('Inhouse milestone deleted successfully');
        } else {
          showError('Cannot delete regular milestones from Inhouse Milestone page. Please delete from Milestone Management.');
          return;
        }
        fetchMilestones(); // Refresh the list
      } catch (error) {
        console.error('Error deleting inhouse milestone:', error);
        showError('Failed to delete inhouse milestone');
      } finally {
              ...
              <div className="flex items-center space-x-3">
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    placeholder="Search Clients, projects, emails, phases..."
                  />
                </div>
              </div>
      ...
      >
        {selectedMilestone && (
          <div className="space-y-6 py-1">
            ...
            {/* Basic Information Card - Single Line Layout */}
...
export default InhouseMilestone;