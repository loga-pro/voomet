...
const Dashboard = () => {
  ...
  const customerPaymentKPIs = [
    ...
    { id: 'total-project-value', title: 'Total Project Value (₹)', value: kpis.financialKPIs.totalProjectValue || '0', icon: CurrencyRupeeIcon, color: 'bg-green-500' },
    { id: 'payment-received', title: 'Payment Received (₹)', value: kpis.financialKPIs.totalPaymentsReceived || '0', icon: CurrencyRupeeIcon, color: 'bg-indigo-500' },
    { id: 'payment-pending', title: 'Payment Pending (₹)', value: kpis.financialKPIs.totalPaymentsPending || '0', icon: CurrencyRupeeIcon, color: 'bg-red-500' }
  ];
  ...
  const vendorPaymentKPIs = [
    ...
    { id: 'total-payments', title: 'Total Payments (₹)', value: (kpis.vendorPaymentKPIs?.totalVendorPayments || 0), icon: CurrencyRupeeIcon, color: 'bg-green-500' },
    { id: 'invoice-raised', title: 'Invoice Raised (₹)', value: (kpis.vendorPaymentKPIs?.totalVendorInvoiceRaised || 0), icon: CurrencyRupeeIcon, color: 'bg-indigo-500' },
    { id: 'pending-payments', title: 'Pending Payments (₹)', value: (kpis.vendorPaymentKPIs?.totalVendorPaymentsPending || 0), icon: ClockIcon, color: 'bg-yellow-500' }
  ];
  ...
  const KpiCard = ({ id, title, value, icon: Icon, color }) => {
    return (
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 hover:shadow-md transition-all">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className={`p-1.5 sm:p-2 rounded-full ${color} bg-opacity-10`}>
              <Icon className={`h-5 w-5 sm:h-6 sm:w-6 ${color.replace('bg-', 'text-')}`} />
            </div>
            <div className="ml-2 sm:ml-3">
              <p className="text-xs font-medium text-gray-600">{title}</p>
              <p className="text-base sm:text-lg font-bold text-gray-900">{Number(value).toLocaleString('en-IN')}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden h-full">
      ...
      {statusKpis && (
        <div className="px-3 sm:px-4 pb-3 sm:pb-4">
          <h4 className="text-xs sm:text-sm font-medium text-gray-700 mb-2 sm:mb-3">Status Summary</h4>
