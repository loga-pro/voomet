...
const VendorPaymentManagement = () => {
  ...
  const fetchPayments = async (page = 1) => {
    try {
      setLoading(true);
      const response = await vendorPaymentsAPI.getAll({ page });
  ...
  const filterPayments = () => {
    ...
    if (filters.vendor) {
      filtered = filtered.filter(payment => 
        payment.vendor.toLowerCase().includes(filters.vendor.toLowerCase())
      );
    }
      ...
      <div className="max-w-none xl:max-w-8xl mx-auto">
          ...
          {viewingPayment ? (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Vendor</label>
                  <p className="mt-1 text-sm text-gray-900">{viewingPayment.vendor}</p>
                </div>
                <div>
        ...
        >
          <div className="space-y-4">
            ...
            Are you sure you want to delete payment record for vendor "{payments.find(p => p._id === paymentToDelete)?.vendor || 'this vendor'}"? This action cannot be undone.
          </p>
